...
contract LendingV2 is Ownable, ReentrancyGuard {
    ...
    function lend(uint _index) public nonReentrant {
        Information storage info = l_createRequest[_index];
        IERC20(info.tokenAddressToBorrow).transferFrom(
            msg.sender,
            info.userRequest,
    ...
    function repay(uint _index) external {
        Information storage info = l_createRequest[_index];
    ...
    function getAllRequest() public view returns (Information[] memory infos) {
        ...
        for (uint i; i < requestIndex; ++i) {
            if (l_createRequest[i].lended == false) {
                unlendedIndex = unlendedIndex.add(1);
            }
        }
        infos = new Information[](requestIndex);
        for (uint i; i < unlendedIndex; ++i) {
            if (l_createRequest[i].lended == false) {
                infos[i] = l_createRequest[i];
            }
        }
    }
    ...
    function getAllDashBoard() public view returns (DashBoard[] memory dash) {
        uint unlendedIndex = 0;
        for (uint i; i < requestDash; ++i) {
            if (l_myDashBoard[i].lender == msg.sender) {
                unlendedIndex = unlendedIndex.add(1);
            }
        }
        dash = new DashBoard[](requestDash);
        for (uint i; i < unlendedIndex; ++i) {
            if (l_myDashBoard[i].lender == msg.sender) {
                dash[i] = l_myDashBoard[i];
            }
        }
    }
